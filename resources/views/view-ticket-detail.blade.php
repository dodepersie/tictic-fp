    ...
    <section>
        <div class="flex flex-col justify-center items-center">
            <div class="container mx-auto max-w-2xl space-y-5 py-10 lg:pt-20">
                <header>
                    <h2 class="text-center text-2xl font-bold text-gray-900 sm:text-3xl">View Ticket Detail</h2>
                </header>

                <form action="{{ route('view-ticket-detail') }}" method="GET" class="w-full">
                        ...
                        <div class="lg:w-1/2">
                            <div class="relative">
                                <input type="text" name="id"
                                    ...
                                    placeholder="Enter your Unique Transaction ID.." value="{{ old('id') }}"
                                    autocomplete="off" />
                ...
                @if (isset($transaction))
                    <div class="mt-5 p-4 border border-gray-200 rounded-lg">
                        <div class="leading-loose">
                            ...
                            <div><strong>Payment Status:</strong>
                                ...
                                @elseif($transaction->status == 'Pending')
                                    <span
                                        ...
                                        <p class="whitespace-nowrap text-sm">{{ $transaction->status }}</p>
                                    </span>
                                @else
                                    <span
                            ...
                            <div><strong>Unique ID:</strong> #{{ $transaction->unique_id }}</div>
                            <div><strong>Ticket Type:</strong> {{ $transaction->ticketType->type }}</div>
                        ...
                        <div class="leading-loose">
                            ...
                            <div><strong>Event Title:</strong> {{ ucfirst($transaction->product->event_title) }}</div>
                            <div><strong>Ticket Price:</strong> IDR
                                {{ number_format($transaction->price, 0, ',', '.') }}
                            </div>
                            <div><strong>Event Start Date:</strong>
                                {{ \Carbon\Carbon::parse($transaction->product->event_start_date)->format('d F Y') }}
                            </div>
                            <div><strong>Event Start Time:</strong>
                                {{ \Carbon\Carbon::parse($transaction->product->event_start_time)->format('H:i') }}
                            </div>
                            <div><strong>Event Address:</strong> {{ $transaction->product->event_address }}</div>
                            <div><strong>Event Location:</strong> {{ $transaction->product->event_location }}</div>
                            <div><strong>Purchased By:</strong> {{ $transaction->user->name }}</div>
                            <div><strong>Order Date:</strong>
                                {{ $transaction->created_at->format('d F Y - H:i:s') }}</div>
                        </div>
                    </div>
                    {{-- @else
