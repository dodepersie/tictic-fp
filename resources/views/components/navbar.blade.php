                        ...
                        <div class="hidden md:flex md:gap-3">
                            ...
                            @auth
                                <div class="flex items-center gap-2">
                                    <div class="hidden lg:block text-sm cursor-default">{{ auth()->user()->name }}</div>
                                    <div x-data="{ isActive: false }" class="relative">
                                        <div class="inline-flex items-center overflow-hidden rounded-md bg-transparent">
                                            <button x-on:click="isActive = !isActive" class="h-full" x-transition>
                                                <span class="sr-only">User Logo</span>
                                                <img src="{{ auth()->user()->profile_picture ? asset('/storage/user_profile/' . auth()->user()->profile_picture) : 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTu9zuWJ0xU19Mgk0dNFnl2KIc8E9Ch0zhfCg&s' }}"
                                                    alt="{{ auth()->user()->name }}"
